...
def graph2DataSource(g):
    ...
    return ColumnDataSource(data = dict(text=text,literal=literals,id=ids,span=span,scope=scopes,category=category))
    ...
    try:
        ...
        for e in g.edges():
            p.line([pos[e[0]][0],pos[e[1]][0]],
                   [pos[e[0]][1],pos[e[1]][1]],line_cap="round",line_width=3,line_alpha=0.4)
            p.diamond([pos[e[1]][0]],[pos[e[1]][1]],alpha=0.4,size=[10])
        p.text(xpos,ypos,text=text,text_color=tcolors,
                   angle=0,text_font_size="12pt",text_align='center',text_baseline='middle')
        p.circle(xpos,ypos,radius=radius,source=source,
        ...
        print(error,": Cannot render graph with %d nodes and %d edges"%(g.number_of_nodes(),g.number_of_edges()))
